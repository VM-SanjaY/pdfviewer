...
class StickyNotesPDFApp:
    ...
    def create_duplicate_window(self):
        ...
        if not self.pdf_document:
            messagebox.showerror("Error", "No PDF is loaded to duplicate.")
            return

        # Create a new Toplevel window
        duplicate_window = Toplevel(self.root)
        duplicate_window.title("Duplicate PDF Viewer")
        duplicate_window.geometry("900x650")
        self.duplicate_windows.append(duplicate_window)

        duplicate_canvas = tk.Canvas(duplicate_window, bg="lightgray", width=900, height=650)
        duplicate_canvas.pack(fill=tk.BOTH, expand=True)
        duplicate_h_scrollbar = tk.Scrollbar(duplicate_window, orient=tk.HORIZONTAL, command=duplicate_canvas.xview)
        duplicate_v_scrollbar = tk.Scrollbar(duplicate_window, orient=tk.VERTICAL, command=duplicate_canvas.yview)
        duplicate_canvas.configure(xscrollcommand=duplicate_h_scrollbar.set, yscrollcommand=duplicate_v_scrollbar.set)
        duplicate_h_scrollbar.pack(side=tk.BOTTOM, fill=tk.X)
        duplicate_v_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        duplicate_pdf_document = self.pdf_document  # Use the currently loaded PDF
        ...
        def render_duplicate_page(page_number):
            """Render the specified page in the duplicate window."""
            if not duplicate_pdf_document:
                messagebox.showerror("Error", "No PDF loaded in duplicate window.")
                return
            ...
            duplicate_canvas.delete("all")
            duplicate_canvas.create_image(0, 0, anchor=tk.NW, image=img_tk)
            duplicate_canvas.img_tk = img_tk
            duplicate_canvas.config(scrollregion=(0, 0, pix.width, pix.height))
        ...
        create_button(toolbar, "🔍 -", zoom_out)
        create_button(toolbar, "🔍 +", zoom_in)
        create_button(toolbar, "⬅️", prev_page)
        create_button(toolbar, "➡️", next_page)
        create_button(toolbar, "🌓", toggle_invert_colors)

        # Render the initial page in the duplicate window
        render_duplicate_page(duplicate_current_page)

        # Close duplicate window logic
    ...
    def close_main_window(self):
        ...
        if self.duplicate_windows:
            messagebox.showwarning(
                "Warning", "Please close all duplicate windows before exiting the main window."
            )
        else:
            self.root.destroy()
