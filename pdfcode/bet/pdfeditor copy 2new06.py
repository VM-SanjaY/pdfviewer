...
class StickyNotesPDFApp:
    ...
    def create_widgets(self):
        ...
        # Create a canvas for scrolling the toolbar
        ...
        # Add horizontal scrollbar for toolbar
        ...
        # Frame inside canvas to hold the toolbar buttons
        toolbar = ctk.CTkFrame(toolbar_canvas)
        
        # Create canvas window to host toolbar
        canvas_window = toolbar_canvas.create_window((0, 0), window=toolbar, anchor="nw")
        
        # Calculate minimum width required for all buttons (23 buttons * (45px width + 3px padding) + navigation)
        min_toolbar_width = 23 * 48 + 150  # 150px for navigation frame
        
        # Configure canvas to update scroll region when toolbar size changes
        def update_toolbar_scroll_region(event=None):
            ...
            # Set minimum content width to ensure scrolling works even when window is narrower
        ...
        # Ensure canvas width updates with window resizing
        def configure_canvas_width(event):
            # Don't let canvas window width be less than minimum required width
            if event.width < min_toolbar_width:
                toolbar_canvas.itemconfig(canvas_window, width=min_toolbar_width)
                # Make sure scrollbar appears
        ...
        # Force the toolbar frame to maintain its width regardless of container
    ...
    # load images in the button
        ...
        # Force buttons to have fixed width
        current_frame = ctk.CTkFrame(toolbar)
        current_frame.pack(side=ctk.LEFT, fill=ctk.Y)
        buttons_in_row = 0
...
####################################### fast and better ##############################################
    def finish_zoom_area(self, event):
        ...
        if expected_pixels > MAX_PIXELS:
            messagebox.showerror("Zoom Error", "Selected area is too small to zoom safely.")
            self.deactivate_selection_mode()
            return
