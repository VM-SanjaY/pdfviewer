...
class StickyNotesPDFApp:
    ...
    def create_widgets(self):
        ...
        button_configs = [
            ...
            # {"type": "custom", "create": lambda parent: self.create_page_entry(parent)},
        ...
        go_button = ctk.CTkButton(toolbar, text="Go", command=self.go_to_page, width=45, fg_color="#00498f", text_color="white")
        go_button.pack(side=ctk.RIGHT, padx=3)

        self.page_entry = ctk.CTkEntry(toolbar, width=100, justify="center")
        self.page_entry.pack(side=ctk.RIGHT, padx=3)
    ...
    def load_pdf(self, file_path=None):
        ...
        try:
            ...
            annotated_url = file_path.replace('.pdf', '_with_annotations.pdf') 
...
####################################################################################################
    ...
    def update_thumbnail_selection(self):
        """Updates the thumbnail selection to highlight the current page."""
        if not self.pdf_document or not self.thumbnail_labels:
            return

        for index, label in enumerate(self.thumbnail_labels):
            if index == self.current_page:
                label.configure(fg_color="blue")  # Highlight selected thumbnail
            else:
                label.configure(fg_color="lightgray")
    ...
    def show_annotated_file(self):
        ...
        if not self.pdf_document or self.current_page is None:
            messagebox.showerror("Error", "No PDF loaded.")
            return
        if "_with_annotations" not in self.pdf_path:
            ...
            if response:
                try:
                    ...
                    if parsed.scheme in ('http', 'https'):
                        try:
                            response = requests.head(annotated_url, allow_redirects=True, timeout=5)
                            if response.status_code == 200 and 'application/pdf' in response.headers.get('Content-Type', ''):
                                self.load_pdf(annotated_url)
                            else:
                    ...
                    elif os.path.isfile(annotated_url) and annotated_url.lower().endswith('.pdf'):
                        self.load_pdf(annotated_url)
                    else:
            ...
            messagebox.showinfo("Info", "You are already viewing the annotated file.")
