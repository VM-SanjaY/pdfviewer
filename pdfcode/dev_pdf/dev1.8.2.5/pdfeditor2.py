...
##########################################################################################################################################################################################
    ...
    def _add_rectangles_to_thumbnail(self, img, page_number, pix_width, pix_height):
        """Add redaction rectangles to thumbnail image with rotation support."""
        ...
        for rect_data in self.all_page_rectangles[page_number]:
            ...
            x1 = rect_data['x1'] * scale_x
            y1 = rect_data['y1'] * scale_y
            x2 = rect_data['x2'] * scale_x
            y2 = rect_data['y2'] * scale_y
            
            fill_color = 'black' if rect_data.get('filled') else None
            outline_color = 'black'
            
            if rotation_angle == 0:
                # No rotation - draw rectangle normally
                if rect_data['filled']:
                    draw.rectangle([x1, y1, x2, y2], fill=fill_color, outline=outline_color, width=1)
                else:
                    draw.rectangle([x1, y1, x2, y2], outline=outline_color, width=1)
            else:
                # Handle rotation - draw as polygon
                corners = [(x1, y1), (x2, y1), (x2, y2), (x1, y2)]
                rotated_corners = []
                for x, y in corners:
                    # Use the same rotate_point method with thumbnail dimensions
                    rx, ry = self.rotate_point(x, y, pix_width, pix_height, rotation_angle)
                    rotated_corners.append((rx, ry))
                
                if rect_data['filled']:
                    draw.polygon(rotated_corners, fill=fill_color, outline=outline_color, width=1)
                else:
                    draw.polygon(rotated_corners, fill=None, outline=outline_color, width=1)
    ...
    def render_page(self, page_number):
        """Render a PDF page to the canvas with redactions applied before display."""
        if not self.pdf_document:
            messagebox.showerror("Error", "No PDF loaded.")
            return
        if self.new_file_loaded == "yes":
        ...
        # Create a draw object to apply redactions directly on the image
        draw = ImageDraw.Draw(img)
        
        # Apply redactions for the current page
        self.page_width_redact, self.page_height_recdact = page.rect.width, page.rect.height
        if not hasattr(self, 'redact_visible'):
            self.redact_visible = True
        if not hasattr(self, 'all_page_rectangles') or self.view_redact == "true":
            self.redraw_black_rectangles_from_api()
            self.redact_visible = True
        if hasattr(self, 'redact_visible') and self.redact_visible:
            self._draw_rectangles_for_current_page(draw=draw)  # Pass draw object to apply redactions
        
        # Convert the redacted image to PhotoImage for Tkinter
        img_tk = ImageTk.PhotoImage(img)
        
        # Clear canvas and display the redacted image
        self.canvas.delete("all")
        self.canvas.create_image(0, 0, anchor="nw", image=img_tk)
        self.canvas.img_tk = img_tk  # Keep a reference to prevent garbage collection
        self.page_width, self.page_height = pix.width, pix.height
        ...
        first_page = self.pdf_document[self.current_page]
        if self.file_opened_for_the_first_time:
            print("self.very_first_rotation_degree------------------", self.very_first_rotation_degree)
            if self.very_first_rotation_degree == 90:
                print("90 degree rotation applied")
                first_page.set_rotation((first_page.rotation + 90) % 360)
        ...
        # Ensure best fit for first render
        if self.rendered_page_count == 0:
            self.root.after(100, self.ensure_best_fit)
            self.rendered_page_count += 1
        self.root.after(0, self.hide_loader)
        ...
        try:
            ...
            if rotation_diff != 0:
                ...
                if rotation_diff == 90:
                    rotated_x = -ty
                    rotated_y = tx
                    # Apply 380 offset based on insertion rotation
                    if insertion_rotation == 0 or insertion_rotation == 180:
                        rotated_x += 380
                        rotated_y += 380
                    elif insertion_rotation == 90 or insertion_rotation == 270:
                    ...
                    rotated_x = -tx
                    rotated_y = -ty
                    # Apply 380 offset for 180-degree rotation
                    if insertion_rotation == 0 or insertion_rotation == 180:
                        rotated_x += 380
                        rotated_y += 380
                    elif insertion_rotation == 90 or insertion_rotation == 270:
            ...
            final_x = max(0, min(final_x, page.rect.width - 20))
            final_y = max(0, min(final_y, page.rect.height - 20))
            ...
            if rotation_diff == 0:
                ...
                text_start_x = final_x
                text_start_y = final_y
                text_rotation = 270  # Counterclockwise rotation for proper orientation
            elif rotation_diff == 180:
                text_start_x = final_x
                text_start_y = final_y
                text_rotation = 180
                ...
                text_start_x = final_x
                text_start_y = final_y
                text_rotation = 90   # Counterclockwise rotation for proper orientation
            else:
                text_start_x = final_x
                text_start_y = final_y
                text_rotation = 0
            
            # Insert each line of text with proper spacing
            for i, line in enumerate(lines):
                line_y_offset = i * adjusted_text_size * 1.2
                
                # Adjust line positioning based on rotation
                if text_rotation == 0:
                    line_x = text_start_x
                    line_y = text_start_y + line_y_offset
                elif text_rotation == 90:
                    line_x = text_start_x - line_y_offset
                    line_y = text_start_y
                ...
                # Ensure line coordinates are within bounds
                line_x = max(0, min(line_x, page.rect.width))
                line_y = max(0, min(line_y, page.rect.height))
