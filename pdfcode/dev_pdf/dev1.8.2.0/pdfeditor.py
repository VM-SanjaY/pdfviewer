...
class StickyNotesPDFApp:
    ...
    def __init__(self, root):
        ...
        if pdf_url:
            ...
            if self.is_freeze == "true":
                self.root.title(f"IDMS Viewer v1.8.2.0 - {self.display_name} {self.file_date_time} - View Only Mode")
            else:
                self.root.title(f"IDMS Viewer v1.8.2.0 - {self.display_name} {self.file_date_time}")
        else:
            self.root.title("IDMS Viewer v1.8.2.0 - No PDF Loaded")
    # window icon 
    ...
    def load_pdf_and_finalize(self, url, start_time):
        try:
            def finalize_after_load():
                ...
                if self.is_freeze == "true":
                    self.root.after(0, lambda: self.root.title(f"IDMS Viewer v1.8.2.0 - {self.display_name} {self.file_date_time} - View Only Mode" ))
                else:
                    self.root.after(0, lambda: self.root.title(f"IDMS Viewer v1.8.2.0 - {self.display_name} {self.file_date_time}"))
                self.hide_loader()
    ...
    # def notify_loader(self, timer_id):
    #     try:
    ...
    #             'Content-Type': 'application/json',
    #             }
    #         response = requests.request("POST", self.loaderurl, headers=headers, data=payload)
    ...
    #     """Toggle visibility of all redaction rectangles (both local and API-fetched)."""
    ...
    #         # Hide all rectangles
    #         self.redacttoggle_button.configure(image=self.icons['show'],fg_color="#00498f",hover_color="#023261")
    #         page = self.pdf_document.load_page(self.current_page)
    #         updated_redact_rectangles = []
            
    #         for rect in list(self.redact_api_rectangles):
    #             if rect['page'] != self.current_page:
    #                 updated_redact_rectangles.append(rect)
    #                 continue
    ...
    #                     annot.delete()
    #                     break
    #             self.change_api_redact_history.append(('delete_annotation', rect))
    #             if rect in self.redact_api_annotations:
    #                 self.redact_api_annotations.remove(rect)
            
    #         self.redact_api_rectangles = updated_redact_rectangles
    #         self.annotation_is_available = bool(self.redact_api_annotations)
    #         self.render_page(self.current_page)
    #         self.redact_visible = False
    #     else:
    #         # Show all rectangles by fetching from API
    #         self.redraw_black_rectangles_from_api()
    #         self.redact_visible = True
    #         self.redacttoggle_button.configure(image=self.icons['hide'],fg_color="#d17a24", hover_color="#b56e26")
...
##########################################################################################################################################################################################
    ...
    def render_thumbnails(self):
        ...
        def load_thumbnails_worker():
            ...
            # Ensure scroll and selection update after all thumbnails are loaded
...
######################################################################################################################################################################################################################
        ...
        for annotation in self.lines:
            if annotation['page'] == self.current_page:
                ...
                for ann in self.annotations:
                    if ann.get("type") == "line" and ann.get("page") == self.current_page and ann.get("id") != line_id:
                        ann['id'] = line_id
    ...
    def redraw_arrows(self, rotation_angle=0):
        ...
        for annotation in self.arrows:
            if annotation['page'] == self.current_page:
                ...
                for ann in self.annotations:
                    if ann.get("type") == "arrow" and ann.get("page") == self.current_page and ann.get("id") != arrow_id:
                        ann['id'] = arrow_id
    ...
    #                 rotated_corners,
    #                 outline=outline_color, fill=fill_color, 
    #                 width=3, tags=("annotation", "red_rectangle"))
        ...
        for entry in reversed(self.change_history):
                ...
                if isinstance(new_ann, tuple) and new_ann[0] == "ellipse":
                    ...
                    if self.is_point_inside_ellipse(click_x, click_y, x1, y1, x2, y2):
                        ...
                        self.annotations = [a for a in self.annotations if not (
                            isinstance(a, tuple) and a[0] == "ellipse" and a[5] == ellipse_id
                        )]
                        # Also remove corresponding original "ellipse" entry if exists
                        self.change_history = [
                            e for e in self.change_history
                            if not (
                                (e[0] == "ellipse" and e[5] == ellipse_id) or
                                (e[0] == "move_annotation" and e[2][5] == ellipse_id)
                            )
                        ]
                        self.render_page(self.current_page)
                        return
            ...
            # Text and Text with Background       
                ...
                if page == self.current_page:
                    ...
                    annotation_entry = annotation_dict.get((page, x_scaled, y_scaled))
    ...
    def deactivate_colour(self, valu, deact=None):
        ...
        # deactiving_function = [self.active_highlight, self.active_deleteannotation, self.active_freehandline, 
        #                        self.active_filledpolygon,  self.active_hollowpolgon, self.active_line, 
        #                        self.active_arrow, self.active_hollowrectangle, self.active_filledrectangle, 
        #                        self.active_hollowellipse, self.active_filledellipse, self.active_redact, 
        #                        self.activedeleteredact_button]
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    #     self.selected_highlight_annot = None  # Store reference to annotation
    ...
    #         self.selected_highlight_annot = clicked_highlight['annotation']  # Store annotation reference
    #         self.start_x = canvas_x
    #         self.start_y = canvas_y
            
    #         # Store original rect but DON'T delete the annotation yet
    #         self.original_highlight_rect = clicked_highlight['rect']
            
    #         # Create temporary visual rectangle
    #         rect = clicked_highlight['rect']
    #         self.temp_highlight_rect = self.canvas.create_rectangle(
    #             rect.x0 * self.zoom_factor, rect.y0 * self.zoom_factor,
    #             rect.x1 * self.zoom_factor, rect.y1 * self.zoom_factor,
    #             outline="red", width=3, fill="yellow", stipple="gray50"
    #         )
            
    #         # Hide the original highlight visually by making it transparent
    #         # This preserves the annotation while showing the temporary one
    ...
    #         # Move the temporary rectangle
    #         current_x = self.canvas.canvasx(event.x)
    #         current_y = self.canvas.canvasy(event.y)
    #         dx = current_x - self.start_x
    #         dy = current_y - self.start_y
    #         self.canvas.move(self.temp_highlight_rect, dx, dy)
    ...
    #     if not self.temp_highlight_rect:
    #         print("No temp highlight rect to finalize")
    #         self.reset_highlight_move_state()
    #         return
        
    #     try:
    #         coords = self.canvas.coords(self.temp_highlight_rect)
    #         if not coords or len(coords) != 4:
    #             print("Invalid coordinates for temp highlight rect")
    #             self.reset_highlight_move_state()
    #             return
    #     except Exception as e:
    ...
    #     rect = fitz.Rect(min(start_x, end_x), min(start_y, end_y), 
    ...
    #     try:
    #         # Now delete the original annotation and create the new one
    #         if self.selected_highlight_annot:
    #             page.delete_annot(self.selected_highlight_annot)
    ...
    #     except Exception as e:
    #         print(f"Error moving highlight: {e}")
    #         # Restore original highlight if move failed
    #         try:
    ...
    #     self.moving_highlight_mode = False
    #     self.selected_highlight_id = None
    #     self.selected_highlight_annot = None  # Clear annotation reference
    #     if hasattr(self, 'original_highlight_rect'):
    #         self.original_highlight_rect = None
...
##############  it moves the highlight from one place to another both the rect cordinate increases in ervery move
    ...
    #         # Store the original drag start position for movement calculation
    #         self.drag_start_x = canvas_x
    #         self.drag_start_y = canvas_y
            
    #         # Store the original canvas coordinates for direct tracking
    #         rect = clicked_highlight['rect']
    #         self.original_canvas_coords = [
    ...
    #         # Delete the original annotation from PDF
    #         page = self.pdf_document[self.current_page]
    #         try:
    #             for annot in page.annots():
    ...
    #         # Create temporary visual rectangle for moving
    #         self.temp_highlight_rect = self.canvas.create_rectangle(
    #             self.original_canvas_coords[0], self.original_canvas_coords[1],
    #             self.original_canvas_coords[2], self.original_canvas_coords[3],
    #             outline="red", width=3, fill="yellow", stipple="gray50"
    #         )
    #         print(f"Starting to move highlight {self.selected_highlight_id}")
    #         print(f"Created temp rect with ID: {self.temp_highlight_rect}")
    #         print(f"Original canvas coords: {self.original_canvas_coords}")
    #     else:
    #         self.moving_highlight_mode = False
    #         self.start_highlight_rectangle(event)
    ...
    # def on_highlight_drag(self, event):
    #     """Handle dragging in highlight mode"""
    #     if self.moving_highlight_mode and self.temp_highlight_rect and hasattr(self, 'original_canvas_coords'):
    #         current_x = self.canvas.canvasx(event.x)
    #         current_y = self.canvas.canvasy(event.y)
            
    #         # Calculate movement delta from the original click position
    #         dx = current_x - self.drag_start_x
    #         dy = current_y - self.drag_start_y
            
    #         # Apply movement to original canvas coordinates
    #         new_coords = [
    #             self.original_canvas_coords[0] + dx,
    #             self.original_canvas_coords[1] + dy,
    #             self.original_canvas_coords[2] + dx,
    #             self.original_canvas_coords[3] + dy
    #         ]
            
    #         # Store current coordinates for finalization
    #         self.current_canvas_coords = new_coords
            
    #         # Update the temporary rectangle position
    #         try:
    #             self.canvas.coords(self.temp_highlight_rect, *new_coords)
    #             print(f"Updated temp rect to: {new_coords}")
    #         except Exception as e:
    ...
    #     try:
    #         # Use the stored coordinates instead of querying the canvas
    #         coords = self.current_canvas_coords
    #         print(f"Using stored coords: {coords}")  # Debug print
    ...
    #     print(f"PDF coords: ({start_x}, {start_y}) to ({end_x}, {end_y})")  # Debug print
        
    #     # Ensure minimum size
    #     if abs(end_x - start_x) < 1 or abs(end_y - start_y) < 1:
    #         print("Rectangle too small, using original size")
    #         original_width = self.original_highlight_rect.x1 - self.original_highlight_rect.x0
    #         original_height = self.original_highlight_rect.y1 - self.original_highlight_rect.y0
    ...
    #     print(f"Before rotation transform: {rect}")  # Debug print
    #     if rotation == 90:
    #         rect = fitz.Rect(rect.y0, page_width - rect.x1, rect.y1, page_width - rect.x0)
    #     elif rotation == 180:
    #         rect = fitz.Rect(page_width - rect.x1, page_height - rect.y1, 
    ...
    #     print(f"After rotation transform: {rect}")  # Debug print
    ...
    #         # Try to restore original highlight
    #         try:
    #             if self.original_highlight_rect:
    #                 restore_highlight = page.add_highlight_annot(self.original_highlight_rect)
    #                 restore_highlight.set_colors(stroke=[1, 1, 0])  # Yellow color
    #                 restore_highlight.set_opacity(0.5)  # Semi-transparent
    #                 restore_highlight.update()
    ...
    #     if hasattr(self, 'original_canvas_coords'):
    #         self.original_canvas_coords = None
    #     if hasattr(self, 'current_canvas_coords'):
    #         self.current_canvas_coords = None
...
######################################################################################################################################
    ...
    #         for page_num in self.page_drawings:
    #             if page_num < len(temp_pdf):
    #                 self._embed_polygons_to_page(temp_pdf[page_num], page_num)
    #         try:
    #             for annotation in self.annotations:
    #                 if isinstance(annotation, tuple) and annotation[0] == 'ellipse' and annotation[1] is not None:
    #                     _, x1, y1, x2, y2, _, mode, page_num = annotation
    #                     self.add_ellipse_annotation(temp_pdf[page_num], x1, y1, x2, y2, mode)
    #         except:
    #             print("Error adding ellipse annotations")
    #         if hasattr(self, "image_overlays"):
    #             for overlay in self.image_overlays:
    #                 page_num = overlay["page"]
    #                 if page_num < len(temp_pdf):
    #                     self.add_image_overlay_to_pdf(temp_pdf[page_num], overlay)
    #         print("terethetttehht reached here .........................")
    #         try:
    #             for rect in self.rectangles:
    #                 if rect['type'] == 'rectangle' and rect['page'] < len(temp_pdf):
    #                     page = temp_pdf[rect['page']]
    #                     rect_coords = fitz.Rect(rect['x1'], rect['y1'], rect['x2'], rect['y2'])
    #                     rgb_color = (1, 0, 0) if rect['color'] == "red" else (0, 0, 0)
    #                     page.draw_rect(rect_coords, color=rgb_color, fill=rgb_color if rect['filled'] else None, width=4 if rect['filled'] else 6)
    #             for line in self.lines:
    #                 if line['type'] == 'line' and line['page'] < len(temp_pdf):
    #                     page = temp_pdf[line['page']]
    #                     start_point, end_point = fitz.Point(line['x1'], line['y1']), fitz.Point(line['x2'], line['y2'])
    #                     page.draw_line(start_point, end_point, color=(1, 0, 0), width=6)
    #             for arrow in self.arrows:
    #                 if arrow['page'] < len(temp_pdf):
    #                     page = temp_pdf[arrow['page']]
    #                     start_point, end_point = fitz.Point(arrow['x1'], arrow['y1']), fitz.Point(arrow['x2'], arrow['y2'])
    #                     annot = page.add_line_annot(start_point, end_point)
    ...
    #         except Exception as e:
    #             print(f"Error adding rectangle annotations: {str(e)}")
    #         for (page_num, x, y), text in self.sticky_notes.items():
    #             self.add_text_sticky_annotation(temp_pdf[page_num], x, y, text)
    #         for (page_num, x, y), annotation_data in self.text_annotations.items():
    #             text = annotation_data["text"] if isinstance(annotation_data, dict) else annotation_data
    #             self.add_plain_text_annotation(temp_pdf[page_num], x, y, text)
    #         for (page_num, x, y), text in self.text_annotations_bg.items():
    #             self.add_text_with_bg_annotation(temp_pdf[page_num], x, y, text)
    #         print("before freehand")
    #         for item in self.change_history:
    #             if item[0] == "freehand":
    #                 _, page_num, _, points = item
    #                 self.add_freehand_line_annotation(temp_pdf[page_num], points)
    #         pdf_buffer = io.BytesIO()
    #         temp_pdf.save(pdf_buffer, garbage=4, deflate=True, deflate_images=True, clean=True)
    #         pdf_buffer.seek(0)
    #         print("before save function-----------------")
    ...
    #             files = {
    #                 'id': (None, self.doc_id),
    #                 'file': (file_name, pdf_buffer, 'application/pdf')
    #             }
    #             response = requests.post(self.server_url, files=files)
    #             if response.status_code in [200, 201]:
    #                 messagebox.showinfo("Success", "File saved successfully")
    ...
    #             print("no redactions svsvsfbbfxxfsdvxfsdfxfxf    elifff")
    #             if len(self.annotation_listed) > 0:
    #                 if "redact_with_annotations" in edit_file_name or "with_annotations" in edit_file_name:
    #                     print("redact_file_name--*****-", edit_file_name)
    #                     upload_file(edit_file_name)
    #                 elif "redact" in self.filename_pdf:
    #                     edit_file_name = edit_file_name.replace("redact", "redact_with_annotations")
    ...
    #         self.canvas.unbind("<ButtonRelease-1>", self.on_polygon_drag_release)
    #         self.canvas.config(cursor="arrow")
    #         self.active_filledpolygon = False
    #         self.filled_polygon_button.configure(text="",fg_color="#00498f",hover_color="#023261")
    #         return
    #     self.active_filledpolygon = True
    #     self.deactivate_colour(self.filled_polygon_button)      
    ...
    #     self.deactivate_colour(self.hollow_polygon_button)
    #     if self.polygon_mode == "hollow":
    #         # Deactivate the mode
    #         self.hollow_polygon_button.configure(text="",fg_color="#00498f",hover_color="#023261")
    #         self.polygon_mode = None
    ...
    #             self.text_annotations[(page_num, x_scaled, y_scaled)]["canvas_id"] = text_container
    #             self.text_annotations[(page_num, x_scaled, y_scaled)]["bbox"] = bbox
    ...
    #     # Split user input into lines based on newlines, then wrap each line if over 30 characters
    #     wrapped_lines = []
    ...
    #         # More accurate width calculation based on font characteristics
    #         # Arial font has roughly 0.5-0.6 character width ratio to font size
    #         actual_text_width = max_line_length * fontsize * 0.6
    #         text_height = fontsize * 1.2 * len(text_lines)
            
    #         # Add padding (5px on sides, 8px extra on bottom)
    #         side_padding = 5
    #         bottom_padding = 8
    ...
    #         self.text_annotations_bg[(page_num, x_scaled, y_scaled)]["canvas_id"] = text_id
    #         self.text_annotations_bg[(page_num, x_scaled, y_scaled)]["bbox"] = bbox
    ...
    #             # Store the bounding box for deletion detection
    #             bbox = self.canvas.bbox(text_id)
    #             self.text_annotations_bg[(page_num, x_scaled, y_scaled)]["canvas_id"] = text_id
    #             self.text_annotations_bg[(page_num, x_scaled, y_scaled)]["bbox"] = bbox
    ...
    def on_line_drag(self, event):
        ...
        if self.dragged_line_id and self.line_drag_start:
            ...
            # Deactivate tools when dragging
            self.deactivate_tools()
            self.line_button.configure(fg_color="#00498f", hover_color="#023261")
            self.arrow_button.configure(fg_color="#00498f", hover_color="#023261")
    ...
    #     if not self.dragged_line_id:
    #         return
            
    #     bbox = self.canvas.bbox(self.dragged_line_id)
    #     print("bbox------",bbox)
    #     if not bbox:
    #         print("Invalid bounding box for dragged line:", bbox)
    #         self.dragged_line_id = None
    #         self.line_drag_start = None
    #         return
            
    #     new_x1, new_y1, new_x2, new_y2 = [v / self.zoom_factor for v in bbox]
    #     # coords = self.canvas.coords(self.dragged_line_id)
    #     # if not coords or len(coords) < 4:
    #     #     print("Invalid coordinates for dragged line:", coords)
    #     #     self.dragged_line_id = None
    #     #     self.line_drag_start = None
    #     #     return
    #     # new_x1, new_y1, new_x2, new_y2 = [v / self.zoom_factor for v in coords[:4]]
        
    #     for line in self.lines:
    #         if line["id"] == self.dragged_line_id and line["page"] == self.current_page:
    #             old_data = dict(line)
    #             line.update({
    #                 "x1": new_x1, "y1": new_y1,
    #                 "x2": new_x2, "y2": new_y2
    #             })
    #             print("Redrawing line:", line["x1"], line["y1"], "to", line["x2"], line["y2"])
    #             self.change_history.append(('move_annotation', old_data, dict(line)))
    #             break

    #     for arrow in self.arrows:
    #         if arrow["id"] == self.dragged_line_id and arrow["page"] == self.current_page:
    #             old_data = dict(arrow)
    #             arrow.update({
    #                 "x1": new_x1, "y1": new_y1,
    #                 "x2": new_x2, "y2": new_y2
    #             })
    #             print("Redrawing arrow:", arrow["x1"], arrow["y1"], "to", arrow["x2"], arrow["y2"])
    #             self.change_history.append(('move_annotation', old_data, dict(arrow)))
    #             break

    #     # Update annotation data
    #     for annotation in self.annotations:
    #         if annotation.get("id") == self.dragged_line_id and annotation["page"] == self.current_page:
    #             annotation.update({
    #                 "x1": new_x1, "y1": new_y1,
    #                 "x2": new_x2, "y2": new_y2
    #             })
    #             print("Redrawing:", annotation["x1"], annotation["y1"], "to", annotation["x2"], annotation["y2"])
    #             break
    ...
    def on_line_release(self, event):
        """Update stored annotation data after dragging."""
        if not self.dragged_line_id or not self.line_drag_start:
            return
        ...
        try:
            ...
            dx = (current_x - self.line_drag_start[0]) / self.zoom_factor
            dy = (current_y - self.line_drag_start[1]) / self.zoom_factor
            ...
            for annotation in self.annotations:
                if annotation.get("id") == self.dragged_line_id and annotation["page"] == self.current_page:
                    ...
                    annotation["x2"] += dx
                    annotation["y2"] += dy
                    print("Redrawing annotation:", annotation["x1"], annotation["y1"], "to", annotation["x2"], annotation["y2"])
                    break

        except Exception as e:
            print("Error during on_line_release:", e)
...
##############################################################################################################
    ...
    #     try:
    #         sql_query = "INSERT INTO redact_info (filename,tempid,coordinate) VALUES (%s, %s, %s)"
    #         self.temp_id = str(uuid.uuid4())
    #         val = (self.filename_pdf, self.temp_id, str(rect_data))
    #         mycursor.execute(sql_query, val)
    #         mydb.commit()
    #         print(mycursor.rowcount, "record inserted.")
    #     except Exception as e:
    #         print(f"Error inserting redact info: {e}")
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    #     # Get page and convert coords to PDF space
    #     page = self.pdf_document.load_page(self.current_page)
    ...
    #     if filled:
    #         annot.set_colors(stroke=(0, 0, 0), fill=(0, 0, 0))  # black border & fill
    #     else:
    #         annot.set_colors(stroke=(0, 0, 0))  # black border only
    ...
    #     self.redact_annotations.append(rect_data)
    #     try:
    #         sql_query = "INSERT INTO redact_info (filename,tempid,coordinate,pagesize,zoomsize) VALUES (%s, %s, %s, %s, %s)"
    #         self.temp_id = str(uuid.uuid4())
    #         page_size = (self.page_width, self.page_height)
    #         val = (self.filename_pdf, self.temp_id, str(rect_data),str(page_size),self.zoom_factor)
    #         mycursor.execute(sql_query, val)
    #         mydb.commit()
    #         print(mycursor.rowcount, "record inserted.")
    #     except Exception as e:
...
# ------------------------------------------------------------------------------------------------------------------------------------------------     
    ...
    def finish_blackrectangle(self, event, filled):
        ...
        #     response = requests.request("POST", redact_url, headers=headers, data=payload)
        #     print("redect_api status",response.text, response.status_code)
        #     # sql_query = "INSERT INTO redact_info (filename, tempid, coordinate, pagesize, zoomsize, doc_id) VALUES (%s, %s, %s, %s, %s, %s)"
        #     # self.temp_id = str(uuid.uuid4())
        #     # page_size = (self.page_width, self.page_height)
        #     # val = (self.filename_pdf, self.temp_id, str(rect_data), str(page_size), self.zoom_factor, str(self.doc_id))
        #     # mycursor.execute(sql_query, val)
        #     # mydb.commit()
        #     # print(mycursor.rowcount, "record inserted.")
        # except Exception as e:
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    #     try:
    #         sql_query = "SELECT coordinate, pagesize, zoomsize FROM redact_info WHERE filename = %s"
    #         mycursor.execute(sql_query, (self.filename_pdf,))
    #         rows = mycursor.fetchall()

    #         for coord_str, saved_pagesize_str, saved_zoom in rows:
    #             # Convert the strings back to usable data
    #             coord = eval(coord_str)  # If you trust your DB data. Use `json.loads` if stored as JSON.
    ...
    #             # Scale the coordinates based on zoom and page size differences
    ...
    #             # Re-add annotation to PDF
    #             page = self.pdf_document.load_page(self.current_page)
    #             rect_pdf = fitz.Rect(x1 / self.zoom_factor, y1 / self.zoom_factor, x2 / self.zoom_factor, y2 / self.zoom_factor)
    #             annot = page.add_rect_annot(rect_pdf)
    #             if coord['filled']:
    ...
    #             # Update local state for undo/redo if needed
    #             rect_data = {
...
## -------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    #     """Load and draw all rectangles from API (both locally drawn and server-stored)."""
    #     if not self.pdf_document or self.current_page is None:
    #         messagebox.showerror("Error", "No PDF loaded.")
    #         return
    #     try:
    ...
    #                 'x2': x2 / self.zoom_factor, 'y2': y2 / self.zoom_factor,
    #                 'filled': coord['filled'], 'id': rect_id, 'color': "black",
    #                 'source': 'api' #coord.get('source', 'api')  # Default to 'api' for server-stored rectangles
    #             }
    #             self.redact_api_rectangles.append(rect_data)
    #             self.redact_api_annotations.append(rect_data)
