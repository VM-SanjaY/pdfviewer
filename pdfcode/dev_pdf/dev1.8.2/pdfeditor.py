...
class StickyNotesPDFApp:
    ...
    def __init__(self, root):
        ...
        if self.is_freeze == "true":
            self.root.title(f"IDMS Viewer v1.8.2 - {self.display_name} {self.file_date_time} - View Only Mode")
        else:
            self.root.title(f"IDMS Viewer v1.8.2 - {self.display_name} {self.file_date_time}")
    # window icon 
    ...
    def handle_url(self, url):
        try:
            ...
            self.loaderurl = f"{part1.split('.com/')[0]}.com/api/v1/samba/close-file-loader/{self.user_id}"
    ...
    def load_pdf_and_finalize(self, url, start_time):
        try:
            def finalize_after_load():
                ...
                if self.is_freeze == "true":
                    self.root.after(0, lambda: self.root.title(f"IDMS Viewer v1.8.2 - {self.display_name} {self.file_date_time} - View Only Mode" ))
                else:
                    self.root.after(0, lambda: self.root.title(f"IDMS Viewer v1.8.2 - {self.display_name} {self.file_date_time}"))
                self.hide_loader()
    ...
    def _log_and_notify_backend(self):
        try:
            ...
            requests.patch(self.loaderurl, headers=headers, json={"id": self.timer_id})
