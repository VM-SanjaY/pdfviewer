    ...
    def load_pdf_and_finalize(self, url, start_time):
        try:
            def finalize_after_load():
                ...
                if self.is_freeze == "true":
                    self.root.after(0, lambda: self.root.title(f"Secondary Viewer v1.8.2 - {self.display_name} {self.file_date_time}  - View only mode"))
                else:
                    self.root.after(0, lambda: self.root.title(f"Secondary Viewer v1.8.2 - {self.display_name} {self.file_date_time}"))
                self.hide_loader()
...
######################################################################################################################################
    ...
    def finalize_move(self, event):
        ...
        try:
            ...
            messagebox.showerror("Error","Failed to add image overlay:",parent=self.root)
