...
class DuplicateStickyNotesPDFApp:
    def __init__(self, root, fileurl):
        ...
        self.root.title(f"Secondary IDMS Viewer")
    ...
    def handle_url(self, url):
        ...
        try:
            ...
            self.root.title(f"Secondary IDMS Viewer - {self.display_name} - {self.file_date_time}")
        except Exception as e:
    ...
    def load_pdf(self, file_path=None):
        ...
        try:
            ...
            try:
                ...
                print("Error in updating thumbnail selection")
...
######################################################################################################################################################################################################################
    ...
    def show_annotated_file(self):
        ...
        response = messagebox.askyesno(
            "Confirm",
            "Are you sure you want to view if this page has annotations? Any unsaved changes will be lost.",parent=self.root
        )
        if not response:
            return
...
######################################################################################################################################
    def save_pdf(self, file_path=None):
        ...
        if not self.pdf_document or self.current_page is None:
            ...
            return
        if self.istempfile:
            messagebox.showinfo("Info", "You cannot save a file within Temp folder.",parent=self.root)
            return
        global edit_file_name
        print("self.annotation_listed",self.annotation_listed)
        print("edit_file_name-----------------",edit_file_name)   
    ...
    #                         messagebox.showerror("Error", "Failed to save the file.")
    #                         return
    #             elif "_redact_with_annotations.pdf" in edit_file_name:
    #                 edit_file_name = edit_file_name
    #                 print("yes redact and annotation")
    #                 files = {'folder_path': (None, folderpath),
    #                         'redact_with_annotations': (edit_file_name, pdf_buffer, 'application/pdf')}
