...
class StickyNotesPDFApp:
    ...
    def __init__(self, root):
        ...
        # Image.MAX_IMAGE_PIXELS = None  # Still set to prevent abrupt crashes
    ...
    def handle_url(self, url):
        ...
        try:
            ...
            self.timer_id = params.get("timer_id", [None])[0]
            ...
            # self.loaderurl = "http://172.20.1.52:8080/api/v1/"+"samba/"+"close-file-loader/"+user_id
            
            print("loader url",self.loaderurl)
            print("user id",user_id)
            print("page number",page_number)
            print("timer_id",self.timer_id)
            ...
            self.action_description = self.display_name + " " + self.file_date_time + " loaded"
            self.root.title(f"IDMS Viewer v1.8.1 - {self.display_name} {self.file_date_time}")
            end_time = time.time()
            self.rendered_time_cal = f"Time taken for pdf to load : {end_time - start_time:.4f} seconds"
            self.handle_url_execution = f"complete time taken for handle_url and load pdf to execute : {end_time - start_time:.4f} seconds"
            print("self.handle_url_execution----------------------------",self.handle_url_execution)
            print("self.action_description----------------------------",self.action_description)
            self.action_description = self.action_description + " - " + self.handle_url_execution
            print("self.action_description----------------------------",self.action_description)
            self.protocol_timer_url = part1.split('.com/')[0] +".com/api/v1/pdf-timer/" + self.timer_id
            self.action_name = "file loading"
            try:           
                threading.Thread(target=self.loggerIssue_function, args=(self.logger_recorder,self.account_name,self.action_name, self.action_description), daemon=True).start()
                print("logger function called")
            except Exception as e:
                print(f"Error in logger_function: {e}")
            try:
                print("protocol_timer_url-----------------------------",self.protocol_timer_url)
                threading.Thread(target=self.protocol_timer, args=(self.protocol_timer_url,"render_time"), daemon=True).start()
                print("protocol_timer function called .....................................64646464646464646464646464646466464646")
            except Exception as e:
                print(f"Error in logger_function: {e}")
            try:
                
                threading.Thread(target=self.notify_loader, args=(self.loaderurl,), daemon=True).start()
                threading.Thread(target=self.protocol_timer, args=(self.protocol_timer_url,"loading_close_time"), daemon=True).start()
            except Exception as e:
                print(f"Error in notify_loader: {e}")
    ...
    def protocol_timer(self, url,from_where):
        try:
            now = datetime.now().isoformat(timespec='milliseconds')
            payload = json.dumps({
                from_where: self.rendered_time_cal if "render_time" in from_where else now
            })
            headers = {
            'Content-Type': 'application/json',
            }
...
######################################################################################################################################################################################################################
    ...
    #     # First, completely clear the root window
    #     for widget in self.root.winfo_children():
    #         widget.destroy()
        
    #     # Reset canvas references to avoid memory issues
    #     self.canvas = None
    ...
    #         # Update page entry and total label
...
######################################################################################################################################
    def save_pdf(self, file_path=None):
        ...
        try:
            ...
            end_time = time.time()
            self.handle_url_execution = f"complete time taken to save a file : {end_time - start_time:.4f} seconds"
            print("self.handle_url_execution----------------------------",self.handle_url_execution)
            print("self.action_description----------------------------",self.action_description)
            self.action_description = self.action_description + " - " + self.handle_url_execution
            print("self.action_description----------------------------",self.action_description)
            self.action_name = "saving file"
            try:
                threading.Thread(target=self.loggerIssue_function, args=(self.logger_recorder,self.account_name,self.action_name, self.action_description), daemon=True).start()
                print("logger function called")
            except Exception as e:
