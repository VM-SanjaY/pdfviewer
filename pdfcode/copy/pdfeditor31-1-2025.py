...
class StickyNotesPDFApp:
    ...
    # self.change_history -  [('highlight', 0, 'fitz-A0'), ('sticky_note', 0, 246.0, 274.0, 'fcghgh'),('add_text_bg', 0, 70.0, 83.0, 'thfy')]]
    # self.change_history -  [('highlight', 0, 'fitz-A0'), ('sticky_note', 0, 246.0, 274.0, 'fcghgh'), ('add_text', 0, 368.0, 290.0, 'rgd')]
    # def undo_change(self):
    #     if not self.change_history:
    #         return
    #     last_action = self.change_history.pop()
    ...
    #         # Remove the text annotation or text with background
    ...
    def redraw_freehand_drawing(self):
        """Redraw lines for the current page with rotation handling."""   
        if not hasattr(self, "page_drawings") or self.current_page not in self.page_drawings:
            return
        page = self.pdf_document.load_page(self.current_page)
        ...
        for line in lines:
            ...
            x1, y1, x2, y2 = [coord * self.zoom_factor for coord in line]
            x1, y1 = self.apply_rotation(x1, y1, rotation_angle, page_rect)
            x2, y2 = self.apply_rotation(x2, y2, rotation_angle, page_rect)

            self.canvas.create_line(x1, y1, x2, y2, fill="black", width=2)

    def apply_rotation(self, x, y, angle, rect):
        """Apply rotation transformation to coordinates based on the page angle."""
        if angle == 90:
            return rect.height - y, x
        elif angle == 180:
 ...
 #####################################        CROPS THE DRAGGED AREA       ############################################
    # def finish_zoom_area(self, event):
    #     """Zoom into the selected area without including surrounding content."""
    ...
    #     # Convert selection coordinates to PDF space
    ...
    #     # Crop the selection
    #     crop_rect = fitz.Rect(x1_pdf, y1_pdf, x2_pdf, y2_pdf)
    #     matrix = fitz.Matrix(self.zoom_factor, self.zoom_factor)
...
###########################################   ZOOMS THE DRAGGED AREA    ###########################################################
    # def finish_zoom_area(self, event):
    #     """Zoom into the selected area and keep it centered."""
    #     if not self.zoom_rectangle:
    #         return
    ...
    #     # Calculate center of selected region before zooming
    #     center_x = ((x1_pdf + x2_pdf) / 2)-50
    #     center_y = ((y1_pdf + y2_pdf) / 2)-45

    #     # Update zoom factor
    #     self.zoom_factor = new_zoom_factor

    #     # Render the page again with the new zoom
    #     self.render_page(self.current_page)

    #     # Adjust scroll region to center the selected area
    #     new_center_x = center_x * self.zoom_factor
    #     new_center_y = center_y * self.zoom_factor

    #     self.canvas.xview_moveto(new_center_x / self.page_width)
    #     self.canvas.yview_moveto(new_center_y / self.page_height)
...
##############################################################################################################
    ...
    #     """Finalize redaction on mouse release."""
    ...
    #     page = self.pdf_document[self.current_page]
    #     page.draw_rect(rect, color=(0, 0, 0), fill=(0, 0, 0))  # Solid black redaction
    #     self.canvas.delete("redaction_preview")
    #     self.render_page(self.current_page)
    #     self.deactivate_tools()


    # def finish_redaction(self, event):
    #     """Finalize redaction on mouse release using OpenCV."""
    #     if not self.redaction_mode:
    #         return
    #     end_x = self.canvas.canvasx(event.x)
    #     end_y = self.canvas.canvasy(event.y)
    ...
    #     img = Image.open(io.BytesIO(pix.tobytes("png")))
    #     img_np = np.array(img)
    #     img_np = img_np.copy()  # Ensure the array is writable
    #     cv2.rectangle(img_np, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 0), thickness=-1)
    #     redacted_img = Image.fromarray(img_np)
    ...
    #     """Remove black rectangles added by OpenCV redaction."""
    #     if not self.pdf_document or self.current_page is None:
    #         messagebox.showerror("Error", "No PDF loaded.")
    #         return
    ...
    #         cv2.rectangle(img, (x, y), (x + w, y + h), (255, 255, 255), thickness=-1)  # Replace with white
    #     img_pil = Image.fromarray(img)
    #     img_bytes = io.BytesIO()
    #     img_pil.save(img_bytes, format='PNG')
    #     page.insert_image(fitz.Rect(0, 0, pix.w, pix.h), stream=img_bytes.getvalue())
    #     self.render_page(self.current_page)
    #     messagebox.showinfo("Success", "Redactions removed successfully.")

    # def finish_redaction(self, event):
    #     """Finalize redaction but do not apply permanently to allow undoing."""
    #     if not self.redaction_mode:
    #         return
    ...
    #     redact_annot = page.add_redact_annot(rect, fill=(0, 0, 0))  # Black box
    #     # page.apply_redactions()  # Apply the redaction permanently
    #     self.redactions.append((self.current_page, redact_annot))
    ...
    def remove_black_boxes(self):
        ...
        if not self.redactions:
            messagebox.showinfo("No Redactions", "No redactions to remove.")
            return

        page = self.pdf_document[self.current_page]

        # Remove only the last redaction from the current page
        for i in range(len(self.redactions) - 1, -1, -1):  # Iterate in reverse to find the most recent one
            ...
            if page_number == self.current_page:
                ...
                while annot:
                    ...
                    if annot.rect == rect:
                        ...
                        self.render_page(self.current_page)
                        messagebox.showinfo("Success", "Last redaction removed successfully.")
