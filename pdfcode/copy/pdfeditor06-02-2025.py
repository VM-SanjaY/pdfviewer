...
######################################################################################################
    ...
    def render_thumbnails(self):
        ...
        self.thumbnails = []
        self.thumbnail_labels = []
        self.thumbnail_cache = {}  # Cache to store loaded thumbnails

        thumbnail_width = 150
        thumbnail_height = 200
        total_pages = len(self.pdf_document)

        def load_thumbnails(start_page=0, batch_size=20):
            """Loads thumbnails in batches asynchronously to prevent UI freeze."""
            for page_number in range(start_page, min(start_page + batch_size, total_pages)):
                ...
                try:
                    ...
                    photo = ImageTk.PhotoImage(img)
                    self.thumbnail_cache[page_number] = photo  # Cache thumbnail
            ...
            # Load the next batch if there are more pages
            if start_page + batch_size < total_pages:
                self.inner_thumbnail_frame.after(100, lambda: threading.Thread(
                    target=load_thumbnails, args=(start_page + batch_size, batch_size)
                ).start())
        ...
        # Start rendering thumbnails in a background thread
        threading.Thread(target=load_thumbnails, args=(0, 20), daemon=True).start()
...
####################################################################################################
    ...
    def redraw_sticky_notes(self):
        """Redraw sticky notes when the page is zoomed or rotated."""
        self.canvas.delete("sticky_note")  # Clear previous notes
        for (page_num, x_scaled, y_scaled), text in self.sticky_notes.items():
            if page_num == self.current_page:
                # Convert back to actual canvas coordinates
                x_position = x_scaled * self.page_width
                y_position = y_scaled * self.page_height
    ...
    def on_canvas_click(self, event):
        """Add a sticky note at the clicked position and embed it correctly."""
        ...
        # Store relative positions to handle zoom & rotation correctly
        x_scaled = x / self.page_width
        y_scaled = y / self.page_height

        page = self.pdf_document[self.current_page]
        
        # Add annotation to PDF
        annot = page.add_text_annot(
            (x_scaled * page.rect.width, y_scaled * page.rect.height,
            (x_scaled * page.rect.width) + 50, (y_scaled * page.rect.height) + 30),
            note_text
        )
        annot.set_opacity(0.85)
        annot.update()
        
        # Store sticky note in dictionary
            ...
            if isinstance(drawing, tuple):
                if len(drawing) == 3:  # Polygon (id, points, color)
                    ...
                    if self.is_point_inside_polygon(x, y, points):
                        ...
                        return
                elif len(drawing) == 2:  # Freehand drawing (id, points)
                    _, points = drawing
