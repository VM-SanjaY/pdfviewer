...
class StickyNotesPDFApp:
    ...
    def __init__(self, root):
        ...
        self.root.title(f"IDMS Viewer v1.8.0 - {self.display_name} {self.file_date_time}")
    ...
    def ensure_thumbnail_selection(self, page_number):
        ...
        if len(self.thumbnails) <= page_number or len(thumb_color) <= page_number:
            print(f"Thumbnails not yet ready. Waiting... (Current: {len(self.thumbnails)}/{len(thumb_color)})")
            # Thumbnails not yet ready, retry after a short delay
            self.root.after(200, lambda: self.ensure_thumbnail_selection(page_number))
        else:
            try:
                print(f"Selecting thumbnail for page {page_number}")
                self._timed_thumbnail_selection(page_number)
            except Exception as e:
                print(f"Error in thumbnail selection: {e}")
                # If there's an error, wait a bit longer and try again
                if len(self.thumbnails) > page_number and len(thumb_color) > page_number:
                    self.root.after(300, lambda: self._timed_thumbnail_selection(page_number))
                else:
                    self.root.after(500, lambda: self.ensure_thumbnail_selection(page_number))
    ...
    def load_pdf(self, file_path=None):
        ...
        try:
            ...
            print("----------------------------------------------------------------------------------------------------------------------------------------------")
            self.root.after(500, lambda: self.ensure_thumbnail_selection(self.current_page))
...
######################################################################################################################################################################################################################
    ...
    def scroll_to_thumbnail(self, selected_label, page_number):
        """Scroll the thumbnail view to make the selected thumbnail visible and prevent it from scrolling back."""
        # Give time for the UI to update before calculating positions
        self.root.after(50, lambda: self._perform_scroll_to_thumbnail(selected_label, page_number))
        
        # Cancel any pending thumbnails scroll reset
        if hasattr(self, "_thumbnail_scroll_reset_id"):
            self.root.after_cancel(self._thumbnail_scroll_reset_id)

    def _perform_scroll_to_thumbnail(self, selected_label, page_number):
        """Actual implementation of thumbnail scrolling with position locking."""
        if not hasattr(selected_label, 'winfo_y'):
            return
            
        total_pages = len(self.pdf_document) if self.pdf_document else 0
        
        # Calculate y_position based on layout
        ...
        # Calculate the scroll fraction to center the thumbnail
        ...
        # Store the current fraction for position locking
        self._last_thumbnail_scroll_position = fraction
        
        # Set up a position check to prevent unwanted scrolling
        self._lock_thumbnail_scroll_position(fraction, 5)  # Check 5 times

    def _lock_thumbnail_scroll_position(self, target_fraction, remaining_checks=5):
        """Ensure the thumbnail scroll position stays at the target location."""
        if remaining_checks <= 0:
            return
            
        current_fraction = self.thumbnail_scrollbar.get()[0]  # Get current scroll position
        
        # If position has changed significantly, restore it
        if abs(current_fraction - target_fraction) > 0.01:
            self.thumbnail_canvas.yview_moveto(target_fraction)
        
        # Schedule the next check with decreasing frequency
        delay = 100 if remaining_checks > 3 else 200
        check_id = self.root.after(delay, lambda: self._lock_thumbnail_scroll_position(target_fraction, remaining_checks - 1))
        self._thumbnail_scroll_check_id = check_id
...
# -----------------------------------------------------------------------------------------------------------
    ...
    def save_pdf_type2(self, file_path=None):
        ...
        elif "_with_annotations" in folderpath:
        ...
        try:
            if len(self.redactions) > 0:
                    ...
                    for rect in self.rectangles:
                        if rect['type'] == 'rectangle' and rect['page'] < len(temp_pdf):
                            ...
                            if rect['filled']:
                                ...
                                page.draw_rect(rect_coords, color=rgb_color, fill=None, width=4)
                except Exception as e:
                ...
                if "redact_with_annotation" not in edit_file_name:
                    redact_file_name = edit_file_name.replace(".pdf", "_redact_with_annotations.pdf")
                else:
                    redact_file_name = edit_file_name
                if len(self.annotation_listed)==0:
                    print("sedgsrxhfxhhfxbxrbxbfxfbbxfxfxfbxcfcnfccnftcrfttg")
                    print("edit_file_name--*****-",edit_file_name)
                    if "redact_with_annotations" in edit_file_name:
                        redact_file_name = edit_file_name
                        print("iffff")
                    elif "with_annotations" in edit_file_name:
                        redact_file_name = edit_file_name.replace("_with_annotations.pdf", "_redact_with_annotations.pdf")
                        print("elifff")
                    elif "_redact.pdf" in edit_file_name:
                        redact_file_name = edit_file_name
                    else:
                        redact_file_name = edit_file_name.replace(".pdf", "_redact.pdf")
                        print("elseee")
                        print("redact_file_name--*****-",redact_file_name)
                ...
                print("redact_file_name--*****-",redact_file_name)
                files = {'folder_path': (None, folderpath),
                        'redact_with_annotations': (redact_file_name, redact_buffer, 'application/pdf')}
                response = requests.post(self.server_url, files=files)
                if response.status_code in [200, 201]:
                    messagebox.showinfo("Success", "File saved successfully")
                else:
                    messagebox.showerror("Error", "Failed to save the file.")
            if len(self.redactions) == 0:
                print("no redactions svsvsfbbfxxfsdvxfsdfxfxf")
                temp_pdf = fitz.open()
                for page_num in range(len(self.pdf_document)):
                    page = self.pdf_document[page_num]
                ...
                try:
                    for rect in self.rectangles:
                        if rect['type'] == 'rectangle' and rect['page'] < len(temp_pdf):
                            ...
                            if rect['filled']:
                                # For filled rectangle
                                page.draw_rect(rect_coords, color=rgb_color, fill=rgb_color, width=3)
                            else:
                                # For hollow rectangle
                ...
                if "_redact.pdf" in filename_pdf: 
                    ...
                    return
                if "redact_with_annotations.pdf" in edit_file_name:
