...
class StickyNotesPDFApp:
    ...
    def create_duplicate_window(self):
        """Create a duplicate window with all changes reflected."""
        if not self.pdf_document:
            messagebox.showerror("Error", "No PDF loaded.")
            return

        duplicate_root = tk.Toplevel()  # Create a new standalone window
        duplicate_root.title("Duplicate PDF Viewer")

        # Create the app for the duplicate window
        duplicate_app = StickyNotesPDFApp(duplicate_root)
        duplicate_app.pdf_document = self.pdf_document
        duplicate_app.current_page = self.current_page
        duplicate_app.sticky_notes = self.sticky_notes.copy()
        duplicate_app.change_history = self.change_history.copy()
        duplicate_app.zoom_factor = self.zoom_factor
        duplicate_app.is_inverted = self.is_inverted
        duplicate_app.thumbnails = self.thumbnails
        duplicate_app.render_thumbnails()
        # Render the page and sync sticky notes and highlights
        duplicate_app.render_page(self.current_page)
        duplicate_app.redraw_sticky_notes()
        ...
        while annot:
            ...
            if annot_type[0] == 8:
                rect = annot.rect
                duplicate_app.highlight_on_canvas(rect)
            annot = annot.next
        threading.Thread(target=duplicate_root.mainloop, daemon=True).start()
    
    def highlight_on_canvas(self, rect):
        """Helper function to display highlights on the canvas."""
        x1, y1 = rect.x0 * self.zoom_factor, rect.y0 * self.zoom_factor
        x2, y2 = rect.x1 * self.zoom_factor, rect.y1 * self.zoom_factor
        self.canvas.create_rectangle(x1, y1, x2, y2, outline="yellow", fill="yellow", stipple="gray50")
